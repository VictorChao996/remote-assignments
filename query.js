...
const checkUserEmail = async function(email){
    ...
    try{
        ...
        console.log("🚀 ~ file: query.js:101 ~ checkUserEmail ~ emailExist:", emailExist)
        return emailExist;
...
const getUserData = async function(id){
    ...
    try{
        ...
        console.log("🚀 ~ file: query.js:136 ~ getUserData ~ userData:", userData)
        // 當找不到userData時
...
function getUserByIdQuery(id){
    return new Promise((resolve, reject) => {
        connection.query('SELECT * FROM user WHERE id = ?', [id], (err, results)=>{
            if(err){
                console.log("🚀 ~ file: query.js:153 ~ connection.query ~ err:", err)
                console.error(err);
                reject(err);
            }
            console.log(results);
